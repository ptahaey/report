...
wiky.process = function(wikitext, options) {
	...
	for (i=0;i<lines.length;i++)
	{
		...
		else if (line.match(/^h1./)!=null && line.match(/$/)!=null)
		{			
			html += "<h1>"+line.substring(3,line.length)+"</h1>";
		} 

		else if (line.match(/^h2./)!=null && line.match(/$/)!=null)
		{
			html += "<h2>"+line.substring(3,line.length)+"</h2>";
		} 

		else if (line.match(/^h3./)!=null && line.match(/$/)!=null)
		{
			html += "<h3>"+line.substring(3,line.length)+"</h3>";
		}

		else if (line.match(/^-- \*/)!=null && line.match(/\*$/)!=null)
		{
			html += "<b>"+line.substring(2,line.length-2)+"</b>";
		}
...
wiky.process_normal = function(wikitext) {
	...
	var url_pattern = /\[(.+?)\|((?:http|https| http| https|ftp|news).+?)\](.+)/;
	while ((myArray = url_pattern.exec(wikitext)) != null)
	{
		wikitext = wikitext.replace(url_pattern, "<a href=\"$2\">$1</a>$3");;
	}
